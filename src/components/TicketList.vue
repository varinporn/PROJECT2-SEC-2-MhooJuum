...
<template>
  <ListModel :is-col="true" :items="ticket" itemWidth="w-[400px]" class="max-h-[500px] overflow-auto px-6 sm:px-6 md:px-8 lg:px-20">
    <template #yourItem="listTicket">
        <button class=" group relative flex rounded-3xl bg-white hover:shadow-lg cursor-pointer" @click="openPopup(listTicket.itemInList)">
          ...
          <div class=" w-1/2 px-3 py-5 space-y-2 text-start">
            ...
            <p class=" font-medium text-sm">{{ listTicket.itemInList.concert.name }}</p>
            <div class=" flex space-x-2">
              <img src="/icons/location.png" alt="" class=" w-4 h-4 mt-[2px]">
              <span class=" text-gray-400 text-xs md:text-sm">{{ listTicket.itemInList.concert.location }}</span>
            </div>
            <div class=" flex space-x-2">
              <img src="/icons/calendar-gray.png" alt="" class=" w-4 h-4 ">
              <span class=" text-gray-400 text-xs md:text-sm">{{ listTicket.itemInList.concert.date }}</span>
            </div>
